...
public class TearableTabPane extends TabPane implements AutoCloseable {
    ...
    protected Skin<?> createDefaultSkin() {
        ...
        skin.closeSplitPane.setOnAction(bindingManager.registerHandler(event -> {
            var splitPane = findParentSplitPane((Button) event.getSource());
            if (splitPane != null) {
                var parentSplitPane = findParentSplitPane(splitPane);
                if (parentSplitPane != null) {
                    //attach orphaned panes to ancestor splitPane before removing their parent.
                    var orphanedPanes = splitPane.getItems().stream()
                            .filter(node -> node instanceof SplitPane)
                            .toList();
                    parentSplitPane.getItems().remove(splitPane);
                    if (!orphanedPanes.isEmpty()) {
                        parentSplitPane.getItems().addAll(orphanedPanes);
                    }
                }
            }
            event.consume();
