...
//                initialSelection.getEndY().doubleValue());
        ...
        this.currentState = new State(LocalDateTime.now().minusDays(1), LocalDateTime.now(), 0, 0);
...
//        formatter.valueProperty().bindBidirectional(stateProperty);
    ...
    private void restoreSelectionFromHistory() {
        if (!history.isEmpty()) {
            XYChartSelection<Date, Number> state = history.pop();
    ...
    private void plotChart(XYChartSelection<Date, Number> currentSelection) {
